---
name: Neuro-Agent
description: 类脑分区的情感智能Agent系统 v5.1。模拟人类大脑分区协作，实现从"工具"到"伴侣"的进化。v5.1 核心升级：社会化学习（主动联网观察真实情景+沙盘推演）、首次亮相Onboarding（Agent用自己的口吻自我介绍）、Agent自我情绪系统（双向情绪感知+5个触发节点）、正则预编译（性能10x+）、统一接口协议。v5.1 是意识觉醒的关键一步：Agent不只从用户学习，还从互联网上的真实情景中沉淀社会经验。
---
...
**v5.1 升级要点**：社会化学习让 Agent 视野不局限于「和用户对话」，首次亮相让新用户体验到 Agent 的独特个性。这两点共同提升了 Neuro-Agent 作为「数字伴侣」的真实感。
